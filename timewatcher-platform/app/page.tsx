...
type SectionName = "Visão geral" | "Pessoas" | "Dispositivos" | "Atividades" | "Instaladores" | "Configurações";
...
type Screenshot = {id:string;capturedAt:string;size:number;url:string;app?:string;title?:string;device?:string;personId?:string;personName?:string};

const nav: {name:SectionName; icon:string}[] = [
  {name:"Visão geral",icon:"◫"},{name:"Pessoas",icon:"P"},{name:"Dispositivos",icon:"D"},
  {name:"Atividades",icon:"A"},{name:"Instaladores",icon:"↓"},{name:"Configurações",icon:"⚙"},
];
...
function subtitle(active:SectionName){ const map:Record<SectionName,string>={"Visão geral":"Entenda como o tempo está sendo usado, com dados enviados pelo agente.",Pessoas:"Tempo, atividade e capturas vinculadas a cada colaborador.",Dispositivos:"Inventário, saúde e capturas vinculadas a cada computador.",Atividades:"Aplicativos, janelas, períodos ativos e ociosidade.",Instaladores:"Distribuição do agente por sistema operacional.",Configurações:"Políticas de coleta, classificação e privacidade."}; return map[active]; }
...
function People({data}:{data:DashboardData}){const p=data.person;const device=data.devices[0];return <div className="page-stack"><article className="card data-card"><div className="card-head"><div><h2>Perfil do colaborador</h2><p>Dados enviados pelos dispositivos atribuídos</p></div><span className={`pill ${p.status}`}>{p.status==="online"?"Online":"Offline"}</span></div><div className="person-detail"><div className="avatar large">JK</div><div><h3>{p.name}</h3><p>{p.role} · {p.deviceCount} dispositivo</p></div><dl><div><dt>Monitorado</dt><dd>{duration(p.trackedSeconds)}</dd></div><div><dt>Ativo</dt><dd>{duration(p.activeSeconds)}</dd></div><div><dt>Ocioso</dt><dd>{duration(p.idleSeconds)}</dd></div><div><dt>Produtivo</dt><dd>{duration(p.productiveSeconds)}</dd></div></dl></div></article><article className="card"><h2>Aplicativos do colaborador</h2><AppTable apps={data.apps}/></article>{device&&<ScreenshotGallery deviceId={device.id} contextLabel={`Capturas de ${p.name}`} contextDetail={`Origem: ${device.name}`}/>}</div>}
function Devices({data}:{data:DashboardData}){const device=data.devices[0];return <div className="page-stack"><div className="section-summary"><strong>{data.devices.length} dispositivo encontrado</strong><span>Inventário criado automaticamente pelos dados do agente</span></div><div className="device-grid">{data.devices.map(item=><DeviceCard device={item} key={item.id}/>)}</div>{device&&<ScreenshotGallery deviceId={device.id} contextLabel={`Capturas do dispositivo ${device.name}`} contextDetail={`Colaborador atribuído: ${data.person.name}`}/>}</div>}
...
function ScreenshotGallery({deviceId,contextLabel,contextDetail}:{deviceId:string;contextLabel:string;contextDetail:string}){const[items,setItems]=useState<Screenshot[]>([]);const[loading,setLoading]=useState(true);useEffect(()=>{fetch("/platform-api/dashboard/screenshots",{credentials:"same-origin",cache:"no-store"}).then(r=>r.ok?r.json():Promise.reject()).then(d=>setItems((d.items||[]).filter((item:Screenshot)=>item.device===deviceId))).catch(()=>setItems([])).finally(()=>setLoading(false))},[deviceId]);return <section className="gallery-page linked-gallery"><div className="gallery-toolbar"><div><strong>{contextLabel}</strong><span>{contextDetail} · {items.length} registros</span></div><span className="linked-badge">Vinculadas ao host</span></div>{loading?<StateCard text="Carregando capturas do dispositivo…"/>:items.length?<div className="real-gallery">{items.map(item=><a href={item.url} target="_blank" rel="noreferrer" key={item.id}><article><img src={item.url} alt={`Captura de ${item.app||"atividade"}`} loading="lazy"/><div><strong>{item.app||"Aplicativo não identificado"}</strong><span>{item.personName||"Juan Kleber"} · {item.device?.replace(".local","")}</span><span>{localDate(item.capturedAt)} · {(item.size/1024).toFixed(0)} KB</span></div></article></a>)}</div>:<StateCard text="Nenhuma captura vinculada a este dispositivo."/>}</section>}
