...
function ProductivityScoreHero({ d, go }: { d: Data; go: (s: Section) => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/platform-api/dashboard/analytics?${query}`, { credentials: "same-origin", cache: "no-store" })
      .then((response) => response.ok ? response.json() : null)
      .then(setReport)
      .catch(() => setReport(null));
  }, [d.period, d.scope, d.tenant.id, d.generatedAt]);
  const score = report?.organization;
  const headline = selfOnly ? "Meu score de produtividade" : managerView ? "Score do meu time" : "Score de produtividade da organização";
  const people = report?.people || [];
  return <section className="score-hero" aria-label={headline}>
    <div className="score-hero-intro"><span>SCORE EXPLICÁVEL · V{report?.scoreMethodology.version || "1.0"}</span><h2>{headline}</h2><p>Uma leitura única de foco, jornada, uso de aplicações, URLs e interação — sem esconder lacunas de coleta.</p><button className="score-hero-link" onClick={() => go("Relatórios")}>Abrir relatório do score →</button></div>
    <div className="score-hero-value"><strong>{score ? score.score.toFixed(1) : "—"}</strong><small>/ 100</small><em className={score?.sufficientData ? "" : "provisional"}>{score?.sufficientData ? scoreConfidence(score) : "Leitura provisória"}</em></div>
    <div className="score-hero-evidence"><div><span>Cobertura da coleta</span><b>{score ? `${score.coverage.toFixed(0)}%` : "Calculando"}</b><i><i style={{ width: `${score?.coverage || 0}%` }} /></i></div><div><span>Base analisada</span><b>{score ? `${score.peopleWithData || 0} de ${score.people || 0} pessoa(s)` : "Aguardando dados"}</b></div><div><span>Atenção necessária</span><b>{score?.peopleInsufficient ? `${score.peopleInsufficient} sem base suficiente` : "Coleta suficiente"}</b></div></div>
    {!selfOnly && <div className="score-hero-ranking"><span>{managerView ? "Destaques do time" : "Melhores scores"}</span>{people.slice(0, 3).map((person) => <div key={person.id}><b>{person.name}</b><strong>{person.productivityScore.score.toFixed(1)}</strong></div>)}{!people.length && <small>Os primeiros dados aparecerão aqui após a sincronização.</small>}</div>}
  </section>;
}
...
function Overview({ d, go, reload }: { d: Data; go: (s: Section) => void; reload: () => void }) {
  ...
  return (
    <>
      ...
      <ProductivityScoreHero d={d} go={go} />
...
function Devices({ d, reload }: { d: Data; reload: () => void }) {
  ...
  async function requestUpdate(host: string) {
    setBusy(host);
    const response = await fetch("/platform-api/dashboard/devices/update", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ host, requestUpdate: true }) });
    const result = await response.json().catch(() => ({}));
    setBusy("");
    if (!response.ok) {
      alert(result.error === "stable_release_not_published" ? `Não há uma versão estável publicada para ${result.platform === "windows" ? "Windows" : "macOS"}.` : "Não foi possível solicitar a atualização.");
      return;
    }
    alert(`Atualização solicitada para ${host}. O agente aplica a versão estável na próxima consulta de atualização.`);
    reload();
  }
  ...
  return (
    <div className="page-stack">
      ...
      <section className="device-list" aria-label="Lista de dispositivos">{shown.length ? shown.map((x) => { const signals = Object.keys(x.signals || {}).length; const person = directory?.people.find((item) => item.host === x.id); const score = person?.productivityScore; const select = () => setSelected(selected === x.id ? null : x.id); return <article className={`device-row${selected === x.id ? " selected" : ""}`} key={x.id} role="button" tabIndex={0} onClick={select} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); select(); } }}><span className="device-os-icon">{x.platform === "macOS" ? "⌘" : "▣"}</span><span className="device-row-identity"><strong>{x.name}</strong><small>{x.inventory?.model || x.platform} · {x.inventory?.sessionUser || "sem sessão"}</small></span><span className="device-row-owner"><small>Responsável</small><strong>{x.personName || "Não vinculado"}</strong></span><span className="device-row-score"><small>Score</small><strong>{score ? score.score.toFixed(1) : "—"}</strong><em>{score ? `${score.coverage.toFixed(0)}% cobertura` : "Sem vínculo"}</em></span><span className="device-row-signal"><small>Coleta</small><strong>{signals}/6 sinais</strong><i><b style={{ width: `${signals / 6 * 100}%` }} /></i></span><span className="device-row-agent"><small>Agente</small><strong>v{x.version || "—"}</strong><em>{x.updateStatus === "current" ? "Atualizado" : x.updateStatus || "Não gerenciado"}</em></span><span className="device-row-seen"><small>Último sinal</small><strong>{date(x.lastSeen)}</strong></span>{canManage && <button type="button" className="device-row-update" disabled={busy === x.id || x.blocked || x.updateStatus === "current"} onClick={(event) => { event.stopPropagation(); requestUpdate(x.id); }}>{busy === x.id ? "Solicitando…" : x.updateStatus === "requested" ? "Solicitada" : x.updateStatus === "current" ? "Atualizado" : "Atualizar"}</button>}<span className={`pill ${x.blocked ? "offline" : x.health === "online" ? "online" : "offline"}`}>{x.blocked ? "Revogado" : HEALTH_LABEL[x.health || x.status]}</span><span className="device-row-arrow">→</span></article>; }) : <State text="Nenhum dispositivo corresponde aos filtros." />}</section><Pager page={safePage} pageCount={pages} total={filtered.length} onPage={setPage} />
...
function DeviceDetail({
  ...
  return (
    <article className={`card device-detail device-workspace${d.blocked ? " blocked" : ""}`}>
      ...
      {tab === "overview" && <div className="device-panel"><div className="device-summary-grid"><div><span>Responsável</span><strong>{d.personName || "Não vinculado"}</strong><small>{d.personEmail || d.inventory?.sessionUser || "Sem identidade"}</small></div><div><span>Última sincronização</span><strong>{date(d.lastSeen)}</strong><small>{d.health === "online" ? "Recebendo dados agora" : "Coleta interrompida"}</small></div><div><span>Uso monitorado</span><strong>{duration(person?.trackedSeconds || 0)}</strong><small>{duration(person?.activeSeconds || 0)} ativo</small></div><div><span>Web e interação</span><strong>{duration(person?.webSeconds || 0)}</strong><small>{(person?.presses || 0).toLocaleString("pt-BR")} teclas · {(person?.clicks || 0).toLocaleString("pt-BR")} cliques</small></div><div className="device-score-summary"><span>Score de produtividade</span><strong>{person?.productivityScore ? person.productivityScore.score.toFixed(1) : "—"}</strong><small>{person?.productivityScore ? `${person.productivityScore.coverage.toFixed(0)}% cobertura · ${scoreConfidence(person.productivityScore)}` : "Vincule o equipamento a uma pessoa"}</small></div></div><div className="device-health-callout"><span className={`health-orb ${health}`}/><div><strong>{health === "online" ? "Equipamento coletando normalmente" : "Equipamento precisa de atenção"}</strong><p>{Object.keys(d.signals || {}).length} de 6 tipos de sinal identificados · agente v{d.version || "desconhecida"} · {d.updateStatus === "current" ? "versão atual" : d.updateStatus || "versão não gerenciada"}</p></div></div></div>}
...
function Installers({ d }: { d: Data }) {
  ...
  async function generateWindowsPackage(download = false) {
    setBusy(true);
    try {
    ...
    <div className="page-stack">
      ...
      <div className="install-grid">
        ...
        <article className="install-card">
          ...
          <p>Baixe um único instalador já vinculado à empresa. Execute-o e aceite a permissão de administrador.</p>
          <button className="download" disabled={busy} onClick={() => generateWindowsPackage(true)}>
            {busy ? "Preparando…" : "Baixar instalador Windows"}
          </button>
        </article>
        <article className="install-card ready">
          ...
          <button onClick={() => generateWindowsPackage(false)}>
